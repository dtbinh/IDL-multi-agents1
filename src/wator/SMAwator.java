	...
	for (int tour = 0; tour < Data.tours; tour++) {
	  ...
	  for (Agent agent : this.agents) {
		if (!containsAgent(deletedAgents, agent)) {
		  ...
		  this.envi = agent.getEnvironnement(); // On met e jour l'environnement
	  ...
	  // Supprimer les requins morts de l'environnement et de la liste des
	  // agents
  ...
  private void printEnv() {
	...
	for (Agent a : this.agents) {
	  ...
	  System.out.println("x: " + x + " | y: " + y + " | "
	      + (this.envi.agentIsPresent(x, y) ? this.envi.getAgentInstance(x, y) : " to be deleted")); // +
	                                                                                                 // this.envi.getAgentInstance(x,
	                                                                                                 // y)
	}
  }
